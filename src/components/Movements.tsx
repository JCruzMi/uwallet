      ...
      {groupedMovements.length === 0 ? (
        ...
        Object.entries(groupedMovements).map(([key, value]: any) =>
          value.map((item: any) => (
            <div
              key={item.id}
              className="flex flex-col sm:flex-row sm:justify-between gap-2  bg-background border border-input p-4 rounded-lg w-full"
            >
            ...
            <AccordionItem value={key} key={key}>
              ...
              <AccordionContent>
                {value.map((item: any) => (
                  <div
                    key={item.id}
                    className="flex flex-col sm:flex-row sm:justify-between gap-2 mb-4 bg-background border border-input p-4 rounded-lg w-full"
                  >
                    <div className="flex gap-2">
                      <div className="md:text-xl text-sm font-semibold">
                        {iconType(item.type)}
